...
out of sync at least once (the ANO001 single-row-gap bridge was added to
contextual.py without a matching update to remediate's copy, so `scan()`
would flag a run that `apply_fixes()` then silently failed to repair; see
CHANGELOG [0.5.0]). Centralizing the presets and masks here means there is
exactly one place to change a threshold or a formula, and detection and
repair cannot disagree about what "stuck", "outlier", or "spike" means
...
def clip_bounds(series: pd.Series, z_thresh: float) -> Tuple[float, float]:
    ...
    Clipping to [L, U] pulls in exactly the flagged outliers and leaves
    every inlier untouched.

    Does not reach an ESD-recovered point (see ``esd_masking_recovery``):
    by construction such a point is inside both the z-band and the IQR
    fence (the reason the z-score and IQR rules both missed it), so
...
def _generalized_esd(
    ...
    ``bound`` is kept for backward-compatible unpacking but is deliberately
    always ``None`` now; see "why not a single band" below. Do not
    resurrect a non-``None`` value here without reading that note: an
    earlier version computed one (from the scale at the step that judged
    the least-extreme flagged point), and it was wrong.

    Why not a single band: Rosner's test is a sequential procedure. At each
    step it removes the single most extreme remaining point and recomputes
    mean/std before testing the next; the number of outliers is then read
    off as the last step index ``j`` for which the test statistic ``R_j``
    exceeded the critical value ``lambda_j``, and every point removed up to
    and including step ``j`` is declared an outlier, whether or not that
    specific point's own ``R_i`` individually cleared ``lambda_i`` at its
    own step. Under heavy, roughly-symmetric contamination (verified: n=200,
    30% contamination at magnitude 4.0, seed 4 in the parameter sweep
    documented in remediate.py's ``_esd_masking_repair``), ``R_i`` is not
    monotonic in removal order: it can be lower for early (most extreme by
    raw magnitude) removals than for later, less extreme ones, because std
    shrinks faster than the removed values' magnitude falls as
    contamination is peeled off. So no single ``(mean, std, critical)``
    triple, neither the boundary step's nor the fully-cleaned post-removal
    step's, is guaranteed to exclude every flagged point: both were tried
    and both left 3 of 62 flagged points inside the band on that fixture.
    There is no per-point fix either, for the same reason: some flagged
    points never individually cleared their own step's critical value, so
    there is no data-derived boundary that is simultaneously outside for
    that point and consistent with why ESD flagged it. See
    ...
    ``masking_suspected``; when True, ``positions`` is folded into the
    points ANO002 flags and into what ``apply_fixes(outliers=...)`` repairs.
    ...
    judges them, dropping ``agreement_count`` to zero exactly when
    contamination is worst, indistinguishable from a harmlessly skewed
    column. ESD removes the most extreme point and recomputes mean/std
    before testing the next, so masking cannot occur by construction.

    Measured against 1,000 clean points with outliers planted at 10 sigma:
    exact at every level (1, 5, 20, 50, 150, 300), and 0 on clean Gaussian
    data where the IQR rule reports 10 false positives.
    ...
    # function of n, i, and alpha alone; it never depends on the data. The
    # original loop called scipy.stats.t.ppf as a scalar call once per
    # removal step (up to max_outliers = 0.4*n times per column), and each
    # call carries real per-call dispatch/validation overhead on top of the
    # actual inversion. On a 3279-row, 26-column benchmark this ppf loop
    # alone accounted for roughly two-thirds of audit_point_anomalies'
    # total runtime (profiled: ~6.9s of a 12.1s scan). Computing the whole
    # criticals array in one vectorized call before the removal loop starts
    # is ~100x faster and produces bit-identical values (verified against
    # the original elementwise loop across n = 50/500/3279/10000).
    ...
    # which is the whole point of the test, so it stays a plain Python loop
    # over already-vectorized per-step numpy ops.
    #
...
def esd_masking_recovery(
    ...
    points a masked column's ANO002 finding covers. Before this was
    centralized, ``masking_suspected`` was computed independently in each
    place, exactly the kind of drift this module exists to prevent (see
    CHANGELOG [0.5.0]).

    "Ambiguous" (the z-score rule finds nothing, IQR finds something) is
    checked first, cheaply; ESD (O(k*n)) only runs when it might change the
    ...
    against a labeled contamination benchmark, that distinguishes real
    masking from an ordinarily skewed column, where ESD and IQR roughly
    agree.
    ...
    EsdRecovery
        ...
        ``esd_positions`` : List[int], positions ESD flagged, or ``[]``
            unless ``masking_suspected`` is True. Gated on
            ``masking_suspected``, not just ``ambiguous``: a bare "z found
            nothing, IQR found something" is also the ordinary signature of
            a harmlessly skewed column, and ESD's own count relative to
            IQR's is what tells the two apart.

    No ``esd_bound``/clip-band field: an earlier version returned one for
    ``remediate.py``'s "clip" repair mode to clip ESD-recovered points to.
    It was statistically unsound; see ``_generalized_esd``'s docstring for
    why no single data-derived band is guaranteed consistent with which
    points Rosner's sequential test actually flags. Repair instead NaNs
    these specific points regardless of ``outliers="clip"``; see
    ``remediate.py``'s ``_esd_masking_repair``.
    """
...
def stuck_run_mask(series: pd.Series, window: int) -> Tuple[pd.Series, pd.Series]:
    ...
    Groups on a bridged view (a single interior NaN interpolated) rather
    than the raw series. A lone missing reading inside an otherwise-flat
    run is still a stuck run; grouping on the raw series would split it in
    two via ``diff()`` reading a NaN as "changed" both at the gap and at
    the row right after it, and neither half might cross ``window`` even
    though the true, uninterrupted run does. Interpolating a single NaN
    only produces a zero diff when both neighbours already agree, so a
    genuine transition (a gap between two different values) still breaks
    the group correctly: this never masks a real change, only bridges a
    real stuck run.
...
def spike_stats(
    ...
    mask, z_scores, flat_context_spike, local_mean, local_std
        ...
        undefined (x / 0), flagged explicitly instead of silently dropped
        as NaN. ``local_mean``/``local_std`` are returned so callers needing
        a repair band can derive ``local_mean +/- threshold * local_std``.
    """
    sq = values.pow(2)
    mp = max(3, window // 2)
