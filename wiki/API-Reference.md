...
Equivalent to `scan()` then `report.apply_fixes()`. `df`, `target`, `time_col`, and `domain` pass through to `scan`; the rest pass through to `apply_fixes`.

**The target label is never repaired.** Pass `target=` so it is protected.

**Limitation:** `fix()` accepts neither `available_at` nor `constraints`, so LEK004 and the validity checks cannot run through this path. Call `scan()` and `apply_fixes()` separately if you need them.
