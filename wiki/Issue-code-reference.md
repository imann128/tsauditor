...
| PNL002 | WARNING | panel | Cross-sectional feature knows the future ordering | Opt-in: drop column | No |
| PNL003 | INFO | panel | Entity too short to audit meaningfully | No | No |
| PNL004 | WARNING | panel | Rows with a null entity id get no checks at all | No | No |
...
Some rows have a null value in `group_col`. `groupby()` drops those rows by default, so they receive **zero** checks (not the panel-level ones, not any per-entity check either), and were previously left untouched by `apply_fixes()` with nothing saying so.

*Evidence:* `n_null_rows`, `n_total_rows`, `pct_null`, `group_col`

*What to do:* Fill in the missing entity id if it's recoverable, or accept that these rows are excluded from auditing and repair entirely. The skip in `apply_fixes()` is now explicit and logged in `last_fixes` as `action: "skip_null_group_rows"`.
...
| -------- | ------- | ----- |
| CRITICAL | Will directly corrupt training or evaluation. Resolve before modeling. | 6 codes |
| WARNING | Worth reviewing. May or may not need action depending on context. | 13 codes |
